...
ElfReader::ElfReader(const char* name, int fd, int offset) :
		name_(name), fd_(fd), phdr_num_(0), phdr_mmap_(0), phdr_table_(0), phdr_size_(0),
			load_start_(0), load_size_(0), load_bias_(0), loaded_phdr_(0){
}

ElfReader::ElfReader(const char* name, int fd, void* base, size_t size,bool bmap) :
		name_(name), fd_(fd), phdr_num_(0), phdr_mmap_(0), phdr_table_(0), phdr_size_(0),
			 load_start_(base), load_size_(size),load_bias_(0),loaded_phdr_(0){
...
ElfReader::~ElfReader() {
	...
	if (phdr_mmap_ != NULL) {
		s_munmap(phdr_mmap_, phdr_size_);
	}
}
...
bool ElfReader::ReadElfHeader() {
	...
	if (rc < 0) {
		debug_printf("can't read file \"%s\": %s\n", name_,
				strerror(errno));
		return false;
	}
	if (rc != sizeof(header_)) {
		debug_printf("\"%s\" is too small to be an ELF executable\n",
				name_);
		return false;
	}
...
bool ElfReader::VerifyElfHeader() {
	if (header_.e_ident[EI_MAG0] != ELFMAG0
		...
		debug_printf("\"%s\" has bad ELF magic %x\n", name_,*(unsigned int*)header_.e_ident);
		return false;
	}

	if (header_.e_ident[EI_CLASS] != ELFCLASS32) {
		debug_printf( "\"%s\" not 32-bit: %d\n", name_,
				header_.e_ident[EI_CLASS]);
		return false;
	}
	if (header_.e_ident[EI_DATA] != ELFDATA2LSB) {
		debug_printf("\"%s\" not little-endian: %d\n", name_,
				header_.e_ident[EI_DATA]);
		return false;
	}

	/*if (header_.e_type != ET_DYN) {
		debug_printf("\"%s\" has unexpected e_type: %d\n", name_,
				header_.e_type);
		return false;
	}*/

	if (header_.e_version != EV_CURRENT) {
		debug_printf("\"%s\" has unexpected e_version: %d\n", name_,
				header_.e_version);
		return false;
	}
...
bool ElfReader::ReadProgramHeader() {
	...
	if (phdr_num_ < 1 || phdr_num_ > 65536 / sizeof(Elf32_Phdr)) {
		debug_printf("\"%s\" has invalid e_phnum: %d", name_,
				phdr_num_);
		return false;
	}
...
bool ElfReader::ReserveAddressSpace() {
	...
	unsigned char* addr = reinterpret_cast<unsigned char*>(min_vaddr);
	...
	load_start_ = start;
	load_bias_ = reinterpret_cast<unsigned char*>(start) - addr;
	return true;
}
	...
	for (size_t i = 0; i < phdr_num_; ++i) {
		...
		if (file_length != 0) {
			...
			if (seg_addr == MAP_FAILED) {
				debug_printf("couldn't map \"%s\" segment %d: %s", name_, i,strerror(errno));
					return false;
			}
		}
		...
		if (seg_page_end > seg_file_end) {
			...
			if (zeromap == MAP_FAILED) {
				debug_printf("couldn't zero fill \"%s\" gap: %s", name_,strerror(errno));
				return false;
			}
		}
...
bool ElfReader::FindPhdr() {
	...
	debug_printf("can't find loaded phdr for \"%s\"", name_);
	return false;
}
...
bool ElfReader::CheckPhdr(Elf32_Addr loaded) {
	...
	debug_printf("\"%s\" loaded phdr %x not in loadable segment",
			name_, loaded);
	return false;
}
