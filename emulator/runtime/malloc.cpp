...
static void init_top(mstate m, mchunkptr p, size_t psize) {
  ...
  int value = psize | PINUSE_BIT;
  uc_err err = uc_mem_write(g_uc,addr,&value, 4);
  /* set size of fake trailing chunk holding overhead space only once */
 // chunk_plus_offset(p, psize)->head = TOP_FOOT_SIZE;
  mchunkptr x = (mchunkptr)((char*)p + psize);
  addr = (int)x + offsetof(malloc_chunk,head);
  value = TOP_FOOT_SIZE;
  ...
  */
  ...
  if (tbase != CMFAIL) {
    ...
    if (nb < m->topsize) { /* Allocate from new or extended top space */
	  ...
	  //set_size_and_pinuse_of_inuse_chunk(m, p, nb);
	  unsigned int addr = (int)r + offsetof(malloc_chunk,head);
	  unsigned int value = rsize | PINUSE_BIT;
	  uc_err err= uc_mem_write(g_uc,addr,&value,4);
...
#define fm gm
    ...
    if (!PREACTION(fm)) {
	  ...
	  if ((char*)p >= fm->least_addr && (value & INUSE_BITS) != PINUSE_BIT) 
	  {
        //size_t psize = chunksize(p);
		size_t psize = value & ~(FLAG_BITS);
        mchunkptr next = chunk_plus_offset(p, psize);
        if (!(value & PINUSE_BIT)) {
          //size_t prevsize = p->prev_foot;
		  size_t prevsize;
		  addr = (int)p + offsetof(malloc_chunk,prev_foot);
		  err = uc_mem_read(g_uc,addr,&prevsize,4);

		  addr = (int)p + offsetof(malloc_chunk,head);
		  err = uc_mem_read(g_uc,addr,&value,4);

          if ((value & INUSE_BITS) == 0) 
		  {
            psize += prevsize + MMAP_FOOT_PAD;
            if (CALL_MUNMAP((char*)p - prevsize, psize) == 0)
              fm->footprint -= psize;
            ...
            if (RTCHECK(ok_address(fm, prev))) { /* consolidate backward */
              if (p != fm->dv) {
                //unlink_chunk(fm, p, prevsize);
				  ...
				  {
					  //unlink_small_chunk(fm, next, nsize)
					  addr = (int)p  + offsetof(malloc_chunk,fd);
					  err = uc_mem_read(g_uc,addr,&value,4);
					  mchunkptr F = (mchunkptr)value;
					  addr = (int)p  + offsetof(malloc_chunk,bk);
					  err = uc_mem_read(g_uc,addr,&value,4);
					  mchunkptr B = (mchunkptr)value;
		...
		addr = (int)next + offsetof(malloc_chunk,head);
		unsigned int value1 = 0;
	    uc_err err = uc_mem_read(g_uc,addr,&value1,4);
        if ((char*)p < (char*)next &&  (value1 & PINUSE_BIT)) 
		{
          if (!(value1 & CINUSE_BIT)) {  /* consolidate forward */
            if (next == fm->top) {
              size_t tsize = fm->topsize += psize;
              fm->top = p;
              //p->head = tsize | PINUSE_BIT;
			  addr = (int)p + offsetof(malloc_chunk,head);
			  value1 = tsize | PINUSE_BIT;
            ...
            else 
			{
              //size_t nsize = chunksize(next);
			  addr = (int)next + offsetof(malloc_chunk,head);
			  err = uc_mem_read(g_uc,addr,&value1,4);
			  value1 &= ~(FLAG_BITS);
			  size_t nsize = value1;
              psize += nsize;
			  ...
			  {
				  //unlink_small_chunk(fm, next, nsize)
				  addr = (int)next  + offsetof(malloc_chunk,fd);
				  err = uc_mem_read(g_uc,addr,&value1,4);
				  mchunkptr F = (mchunkptr)value1;
				  addr = (int)next  + offsetof(malloc_chunk,bk);
				  err = uc_mem_read(g_uc,addr,&value1,4);
				  mchunkptr B = (mchunkptr)value1;

				  bindex_t I = small_index(nsize);
				  //assert(next != B);
				  //assert(next != F);
				  //assert(chunksize(next) == small_index2size(I));
             ...
             // set_size_and_pinuse_of_free_chunk(p, psize);
			  addr = (int)p  + offsetof(malloc_chunk,head);
			  value1 = (psize|PINUSE_BIT);
			  err = uc_mem_write(g_uc,addr,&value1,4);
			  
			  addr = (int)p  + psize + offsetof(malloc_chunk,prev_foot);
			  value1 = psize;
			  err = uc_mem_write(g_uc,addr,&value1,4);
              if (p == fm->dv) {
                fm->dvsize = psize;
          ...
          else
		  {
			  ...
			  uc_err err = uc_mem_read(g_uc,addr,&value1,4);
			  value1 &= (~PINUSE_BIT);
			  uc_mem_write(g_uc,addr,&value1,4);
          ...
          if (is_small(psize)) 
		  {
            ...
            //check_free_chunk(fm, p);
			  ...
			  mchunkptr B = smallbin_at(fm, I);
			  mchunkptr F = B;
			  assert(psize >= MIN_CHUNK_SIZE);
			  if (!smallmap_is_marked(fm, I))
				  mark_smallmap(fm, I);
			  else if (RTCHECK(ok_address(fm, B->fd)))
			  F = B->fd;
			  else 
			  {
			  CORRUPTION_ERROR_ACTION(fm);
