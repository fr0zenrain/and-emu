...
unsigned char atomic_code[] =
{
        
};

int emulator::init_vectors()
{
    unsigned int spec_addr = 0xFFFF0000;
    unsigned int atomic_addr = 0xffff0f60;
    uc_err err = uc_mem_map(uc,spec_addr, 0x1000, UC_PROT_ALL);
    if(err != UC_ERR_OK)
    {
        printf("Failed on uc_mem_map() with error returned: %u\n", err);
        return 0;
...
int emulator::init_fake_soinfolist(){
    //i am dalvik vm
    uc_err err;
    soinfo* dl = (soinfo*)sys_malloc(sizeof(soinfo));
    soinfo* dvm = (soinfo*)sys_malloc(sizeof(soinfo));
    if (dl == NULL || dvm == NULL) {
        return 0;
    }

    uc_mem_write(g_uc, (uint64_t)dl, "libdl.so", 9);
    uc_mem_write(g_uc, (uint64_t)dvm, "libdvm.so", 10);
    int addr = (int)dl + offsetof(soinfo, next);
